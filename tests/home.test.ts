...
test('index page has expected meta description', async ({ page }) => {
  ...
  await expect(metaDescription).toHaveAttribute('content', '2000+ SVG icons from Font-Awesome for a Svelte 4, 5, and Runes project.');
});

test('index page has expected meta keywords', async ({ page }) => {
  const metaKeywords = page.locator('meta[name="keywords"]');
  await expect(metaKeywords).toHaveAttribute('content', 'svg, font-awesome, icons, svelte, runes, accessible, ide support, faster compiling, css frameworks support, dark mode support');
});
...
test('index page has expected meta og', async ({ page }) => {
  ...
  await expect(metaOgDescription).toHaveAttribute('content', '2000+ SVG icons from Font-Awesome for a Svelte 4, 5, and Runes project.');
...
test('index page has expected meta twitter', async ({ page }) => {
  ...
  await expect(metaTwitterDescription).toHaveAttribute('content', '2000+ SVG icons from Font-Awesome for a Svelte 4, 5, and Runes project.');
