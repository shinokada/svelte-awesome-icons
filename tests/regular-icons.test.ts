...
test('Regular Icons page has expected h1, meta title', async ({ page }) => {
  await expect(page.getByRole('heading', { name: title })).toBeVisible();
});

test('Regular Icons page has expected meta title', async ({ page }) => {
  await expect(page).toHaveTitle(title);
});

test('Regular Icons page has expected meta description', async ({ page }) => {
  const metaDescription = page.locator('meta[name="description"]');
  await expect(metaDescription).toHaveAttribute('content', description);
});

test('Regular Icons page has expected meta og', async ({ page }) => {
  ...
  await expect(metaOgUrl).toHaveAttribute('content', 'http://localhost:4173/regular-icons');
...
test('Regular Icons page has expected meta twitter', async ({ page }) => {
