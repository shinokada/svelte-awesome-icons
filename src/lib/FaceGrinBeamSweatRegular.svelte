<script lang="ts">
  ...
  let { size = ctx.size || '24', role = ctx.role || 'img', color = ctx.color || 'currentColor', ariaLabel = 'face grin beam sweat regular,', class: classname, ...restProps } = $props<Props>();
</script>
