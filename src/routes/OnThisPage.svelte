<script lang="ts">
  ...
  import { AngleDownSolid } from '$lib';
  interface LinkType {
    name: string;
    href?: string;
    rel?: string;
...
<div
  class="xl:hidden z-20 fixed top-20 p-2 right-6 flex dark_bg_theme dark:text-white on-this-page"
>
  On this page
  <AngleDownSolid class="flex-inline w-5 h-5 ms-2 mt-1 text-white dark:text-white" />
</div>
<div use:init>
  {#if headings.length}
    <Dropdown
      triggeredBy=".on-this-page"
      containerClass="w-64 z-20 dark_bg_theme border border-gray-200 dark:border-gray-700 rounded-lg "
    >
